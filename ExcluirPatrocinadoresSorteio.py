...
class ExcluirPatroc:
    ...
    def seguir(self, pagina):
        driver = self.driver
        driver.get("https://www.instagram.com/" + pagina)
        time.sleep(2)

        follower_element = "//a[@href='/" + pagina + "/following/']"
        followers = driver.find_element_by_xpath(follower_element)
        followers.click()
        time.sleep(2)
...
bot = ExcluirPatroc('janliucas', 'deatherinferno')
bot.login()
bot.seguir("agenciafrannof")
